...
Функция ЗначениеКритичностиСтрокой(описаниеОшибки)
	...
	Иначе
		...
		ИначеЕсли описаниеОшибки.Серьезность = "MINOR" Тогда
			Значение = "Information";
		ИначеЕсли описаниеОшибки.Серьезность = "MAJOR" Тогда
			Значение = "Warning";
		Иначе
			Значение = "Error";
		КонецЕсли;
...
Функция ПодготовитьОшибкиАПК(РезультатыПроверки, МассивГруппировок)
	...
	Для Каждого ПутьКОбъекту Из МассивГруппировок Цикл
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			...
			КонецЕсли;
			// ++ Alkoko исправление ошибки java.lang.IllegalArgumentException: Start pointer [line=1, lineOffset=0] should be before end pointer [line=1, lineOffset=0]
			ПроверкаПройдена = ПроверкаПройдена
				И (НайденнаяСтрока.НачальнаяСтрока <> НайденнаяСтрока.КонечнаяСтрока
					ИЛИ НайденнаяСтрока.НачальнаяКолонка <> НайденнаяСтрока.КонечнаяКолонка
					ИЛИ НайденнаяСтрока.КонечнаяКолонка > 1);
			// -- Alkoko исправление ошибки java.lang.IllegalArgumentException: Start pointer [line=1, lineOffset=0] should be before end pointer [line=1, lineOffset=0]
...
Функция ОбработатьОшибкиАПК(ВыборкаОшибокАПК, РезультатыПроверки, МассивГруппировок)
	...
	Пока ВыборкаОшибокАПК.Следующий() Цикл
		...
		Результат.ЗатрачиваемыеУсилия = описаниеОшибки.ЗатрачиваемыеУсилия;
